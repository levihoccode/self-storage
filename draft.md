...
- Một số điều chưa rõ:
  + Nếu khách muốn mở rộng khoang chứa, số lượng khoang chứa thì sao? Giải pháp dự kiến: thuê nhiều hợp đồng
  + Nếu khách muốn chỉ định cụ thể 1 khoang chứa? Giải pháp dự kiến: Không cho phép
...
- **FM**
  - Các yêu cầu đặt khoang chứa sẽ được liệt kê ở một trang và có các nút (button) để thao tác (details, response, update status, ...), một entry trong list bao gồm các thông tin:
    + customer_email
    + customer_phone
    + unit_type
    + start_date (MM/DD/YYYY)
...
- NOTES:
  - Quá trình duyệt yêu cầu có thể tự động.
...
**Schema dự tính:**
- RentalRequest: chứa các thông tin được gửi từ form trên website. (các fields của bảng giống entry trong list yêu cầu của FM)
- RentalOrder: chứa các thông tin đơn hàng đã được `Approve` từ FM, bao gồm các thông tin:
  - customer_id (1 - N: Account(role="customer")) - nullable -> được gán ngay sau khi khách tạo tài khoản
  - staff_id (N - 1: Account(role="fs"), null until the FS confirm and status pending -> in progress)
  - unit_type
  - start_date - Thời gian bắt đầu thuê (MM/DD/YYYY)
  - period - Thời gian thuê (month)
  - unit_id
  - cancel_reason
  - status: 
...
- Tạo hóa đơn và yêu cầu chọn lịch cho tài khoản.
